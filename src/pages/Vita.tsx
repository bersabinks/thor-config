import { useEffect, useState } from 'react'
import { DeviceStatus } from '../components/DeviceStatus'
import { ExecutionLog } from '../components/ExecutionLog'
import { VitaModule } from '../components/VitaModule'
import type { AdbDevice } from '../../electron/main/adb/types'
import { useSettings } from '../store/settings'

export function Vita() {
  const [device, setDevice] = useState<AdbDevice | null>(null)
  const { simulationMode } = useSettings()

  useEffect(() => {
    let active = true

    async function poll() {
      try {
        const devices = await window.electronAPI.adb.listDevices()
        if (active) setDevice(devices[0] ?? null)
      } catch {
        if (active) setDevice(null)
      }
    }

    poll()
    const interval = setInterval(poll, 2000)
    return () => {
      active = false
      clearInterval(interval)
    }
  }, [simulationMode])

  return (
    <div className="page">
      <div className="page-header">
        <div>
          <h2>PS Vita</h2>
          <p className="page-desc">
            Convertit automatiquement les archives PS Vita déposées dans le dossier d'import au
            format Vita3K, puis les livre vérifiées sur la console ou sur le PC.
          </p>
        </div>
        <DeviceStatus />
      </div>

      <div className="page-content page-content--fill">
        <VitaModule device={device} />
        <ExecutionLog />
      </div>
    </div>
  )
}
